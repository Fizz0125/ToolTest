...
class WapPay extends \WxPayNotify
{
    ...
    public function __construct()
    {
        ...
        $wx_payment_info['wx_appid'] = env('WAP_WX_APPID','');
        $wx_payment_info['wx_mchid'] = env('WAP_WX_MCHID','');
        $wx_payment_info['wx_appsecret'] = env('WAP_WX_APPSECRET','');
        $wx_payment_info['wx_pay_sign_key'] = env('WAP_WX_PAY_SIGN_KEY','');
        $wx_payment_info['wx_notify_url'] = env('WAP_WX_NOTIFY_URL','');
     ...
     */
	public function WxJsApiPay($orderData = [])
    {
        $can_pay = false;
        $data = [];
        if (!empty($orderData)) {
            ...
            if ($pay_keys_bool) {
                ...
                if(isset($order['result_code']) && $order['result_code'] == 'SUCCESS'){
                    ...
                    } else if(isset($order['err_code'])){
                        $this->error = $order['err_code'];
                    } else {
    ...
    /**
     * [WxBrowserPay 微信外浏览器支付]
     * @author Edge
     * @time 2017/10/15
     * @param  array  $orderData [description]
     */
    public function WxBrowserPay($orderData = [])
    {
        $can_pay = false;
        $data = [];
        if (!empty($orderData)) {
            $pay_keys_bool = true;
            foreach ($this->pay_keys as $key => $value) {
                if (!isset($orderData[$value]) && $value != 'openid') {
                    $this->error = $value . '参数必传';
                    $pay_keys_bool = false;
                    break;
                }
