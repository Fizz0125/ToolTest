...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url($wx_payment_info['wx_notify_url']);//异步通知url
		}
		$inputObj->SetAppid($wx_payment_info['wx_appid']);//公众账号ID
		$inputObj->SetMch_id($wx_payment_info['wx_mchid']);//商户号
		$client_ip = get_client_ip();
		$client_ip = $client_ip ? $client_ip : $_SERVER['REMOTE_ADDR'];
		$inputObj->SetSpbill_create_ip($client_ip);//终端ip	  
