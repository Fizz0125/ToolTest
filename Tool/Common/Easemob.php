...
//		$this->org_name      = isset ( $options ['org_name'] )      ? $options ['org_name']      : 'thetiger';
//
//		//本地、测试环境
//		if($_SERVER['SERVER_ADDR'] == '127.0.0.1' || $_SERVER['SERVER_ADDR'] == '119.23.146.27'){
//			$this->client_id     = isset ( $options ['client_id'] )     ? $options ['client_id']     : 'YXA6hMwpMBa6Eee5bhuj7pVTyg';
//			$this->client_secret = isset ( $options ['client_secret'] ) ? $options ['client_secret'] : 'YXA6Q5Lnd81a3_eBigLlxGZMCU77ad8';
//			$this->app_name      = isset ( $options ['app_name'] )      ? $options ['app_name']      : 'ldd4test';
//		}
//		//正式环境
//		/*if($_SERVER['SERVER_ADDR'] == '119.23.146.27'){
//			$this->client_id     = isset ( $options ['client_id'] )     ? $options ['client_id']     : 'YXA6bMDrABa6Eee8VwMdTATJBg';
//			$this->client_secret = isset ( $options ['client_secret'] ) ? $options ['client_secret'] : 'YXA6Za7DFa05uXhP5sjRpHFG5pJ6Mg4';
//			$this->app_name      = isset ( $options ['app_name'] )      ? $options ['app_name']      : 'ldd';
//		}*/
//
...
//		}
        ...
        $this->client_id     = $options ['client_id']??config('localsystems.im.client_id');
        $this->client_secret = $options ['client_secret']??config('localsystems.im.client_secret');
        $this->org_name      = $options ['org_name']??config('localsystems.im.org_name');

        //测试环境
        //if($_SERVER['SERVER_ADDR'] == '120.76.225.40'){
        //	$this->app_name = isset ( $options ['app_name'] ) ? $options ['app_name'] : 'ziwork4test';
        //}
        //正式环境
        //if($_SERVER['SERVER_ADDR'] == '120.24.16.222'){
        $this->app_name = $options ['app_name']??config('localsystems.im.app_name');
        //}
	...
	*/
	...
	function postCurl($url,$body,$header,$type="POST"){
		...
		//3.抓取URL并把它传递给浏览器
		$res=curl_exec($ch);
        $statuscode = curl_getinfo($ch,CURLINFO_HTTP_CODE);//添加状态码获取
