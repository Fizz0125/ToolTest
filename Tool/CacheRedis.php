...
class CacheRedis
{
    ...
    {
        if(empty($options)){
            $options['host'] = env('CACHE_REDIS_HOST','');
            $options['port'] = env('CACHE_REDIS_PORT','');
            $options['auth'] = env('CACHE_REDIS_AUTH','');
            $options['persistent'] = env('CACHE_REDIS_PERSISTENT','');
        }
    ...
    public function _connect()
    {
        if (empty($this->_redis)) {
            try {
                ...
                if (env('APP_ENV') == 'pro' || env('APP_ENV') == 'test' || env('APP_ENV') == 'stg') {
